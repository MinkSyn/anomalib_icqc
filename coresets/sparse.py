...
class SparseRandomProjection:
    ...
    def _sparse_random_matrix(self, n_features):
        ...
        if density == 1:
            ...
            components = binomial.sample(
                (self.n_components, n_features)) * 2 - 1
            ...
            components = torch.zeros((self.n_components, n_features),
                                     dtype=torch.float64)
            for i in range(self.n_components):
                # find the indices of the non-zero components for row i
                nnz_idx = torch.distributions.Binomial(total_count=n_features,
                                                       probs=density).sample()
                ...
                data = torch.distributions.Binomial(
                    total_count=1,
                    probs=0.5).sample(sample_shape=c_idx.size()) * 2 - 1
